...
function LinkList({ project }: { project: Project }) {
  ...
  return <section className="detail-section" aria-labelledby="links-heading"><span className="eyebrow">PUBLIC REFERENCES</span><h2 id="links-heading">Continue exploring</h2><div className="public-links">{links.map((link) => <a key={link.id ?? link.url} href={link.url} target="_blank" rel="noreferrer" className="public-link">{link.label || (link.kind === 'github' ? 'GitHub repository' : 'Trello board')} <span aria-hidden="true">↗</span></a>)}</div></section>
}

function Gallery({ project, onSelect }: { project: Project; onSelect: (media: Media) => void }) {
  const images = imageMedia(project)
  if (!images.length) return null
  return <section className="detail-section" aria-labelledby="gallery-heading"><span className="eyebrow">CURATED MEDIA</span><h2 id="gallery-heading">Visual evidence</h2><div className="detail-gallery">{images.map((media) => <button type="button" className="gallery-item" key={media.id ?? media.source} onClick={() => onSelect(media)}><img src={localMediaURL(media.source)} alt={media.alt_text || `${project.name} ${media.role}`} /><span>{media.caption || (media.role === 'thumbnail' ? 'Open original' : media.role)}</span></button>)}</div></section>
}

function VideoReferences({ project }: { project: Project }) {
  const videos = project.media.filter((media) => media.role === 'video')
  if (!videos.length) return null
  return <section className="detail-section" aria-labelledby="video-heading"><span className="eyebrow">DEMO</span><h2 id="video-heading">Watch the workflow</h2><div className="video-list">{videos.map((video) => /^https:\/\//i.test(video.source) ? <a className="video-reference" key={video.id ?? video.source} href={video.source} target="_blank" rel="noreferrer">{video.caption || 'Open public video'} ↗</a> : <video className="local-video" key={video.id ?? video.source} controls preload="metadata" aria-label={video.caption || `${project.name} demo`}><source src={localMediaURL(video.source)} /><track kind="captions" /></video>)}</div></section>
}
