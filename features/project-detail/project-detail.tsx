...
function AgenticPlatform({ value }: { value: string }) {
  ...
  return <p className="agentic-platform-value">{parts.map((part, index) => /^https:\/\//i.test(part) ? <a key={`${part}-${index}`} href={part} target="_blank" rel="noreferrer">{part}</a> : part)}</p>
}
...
export function ProjectDetail({ projectID }: { projectID: string }) {
  ...
  return <main className="detail-page"><a href="/" className="back-link">← Back to catalog</a><header className="detail-hero"><span className="eyebrow">PROJECT / {project.id.slice(0, 8)}</span><h1>{title}</h1>{project.description && <p>{project.description}</p>}</header><div className="detail-layout"><div className="detail-primary"><Gallery project={project} onSelect={setSelected} /><VideoReferences project={project} />{milestones.length > 0 && <section className="detail-section" aria-labelledby="history-heading"><span className="eyebrow">MANUAL HISTORY</span><h2 id="history-heading">The work, over time</h2><ol className="timeline">{milestones.map((milestone) => <li key={milestone.id ?? `${milestone.date}-${milestone.title}`}><time dateTime={milestone.date}>{milestone.date}</time><div><h3>{milestone.title}</h3><p>{milestone.description}</p></div></li>)}</ol></section>}</div><aside className="detail-aside"><section className="detail-section metadata-section"><span className="eyebrow">REGISTERED FIELDS</span>{project.technologies.length > 0 && <div className="field-group"><h2>Technologies</h2><ul className="technology-list">{project.technologies.map((technology) => <li key={technology}>{technology}</li>)}</ul></div>}{project.agentic_platform && <div className="field-group agentic-platform"><h2>Agentic platform</h2><AgenticPlatform value={project.agentic_platform} /></div>}</section><LinkList project={project} /></aside></div>{selected && <div className="lightbox" role="dialog" aria-modal="true" aria-label="Expanded project image" onClick={() => setSelected(null)}><div className="lightbox-content" onClick={(event) => event.stopPropagation()}><button type="button" className="lightbox-close" aria-label="Close image" onClick={() => setSelected(null)}>×</button><img src={localMediaURL(fullSizeSource(selected, project.media))} alt={selected.alt_text || `${project.name} expanded`} />{selected.caption && <p>{selected.caption}</p>}</div></div>}</main>
}
