...
const emptyProject: ProjectForm = { name: '', description: '', agentic_platform: '', github_repository_url: '', trello_backlog_url: '', technologies: [], links: [], media: [], milestones: [] }
...
export function Administration() {
  ...
  const refresh = async (id?: string) => {
    ...
    if (id) { const loaded = await getProject(id); setProject(loaded); setForm({ name: loaded.name, description: loaded.description || '', agentic_platform: loaded.agentic_platform || '', github_repository_url: loaded.github_repository_url || '', trello_backlog_url: loaded.trello_backlog_url || '', technologies: loaded.technologies, links: loaded.links, media: loaded.media, milestones: loaded.milestones }) }
  }
  useEffect(() => { const id = new URLSearchParams(window.location.search).get('project') ?? undefined; void refresh(id).catch((cause: Error) => setError(cause.message)) }, [])
  ...
  return <main className="admin-page"><header className="admin-header"><div className="admin-header-nav"><a href="/">← Back to dashboard</a><span className="eyebrow">LOCAL / ADMINISTRATION</span></div><h1>Inventory Administration</h1><p>Safe metadata, approved media, and manual history. No login required.</p></header>
    ...
    {message && <p className="success-message" role="status">{message}</p>}{error && <p className="form-error" role="alert">{error}</p>}
     <form className="admin-card" onSubmit={save}><span className="eyebrow">01 / PROJECT METADATA</span><h2>{project ? 'Edit project' : 'Create project'}</h2><label>Name *<input required aria-label="Project name" value={form.name} onChange={(event) => setField('name', event.target.value)} /></label><label>Description<textarea aria-label="Project description" value={form.description} onChange={(event) => setField('description', event.target.value)} /></label><label>Agentic platform<input value={form.agentic_platform} onChange={(event) => setField('agentic_platform', event.target.value)} /></label><label>GitHub repository URL<input type="url" aria-describedby="repository-help" value={form.github_repository_url} onChange={(event) => setField('github_repository_url', event.target.value)} placeholder="https://github.com/org/repository" /></label><p id="repository-help" className="helper-text">Optional. Use a public HTTPS GitHub repository URL.</p><label>Trello backlog URL<input type="url" aria-describedby="backlog-help" value={form.trello_backlog_url} onChange={(event) => setField('trello_backlog_url', event.target.value)} placeholder="https://trello.com/b/board" /></label><p id="backlog-help" className="helper-text">Optional. Use a public HTTPS Trello backlog URL.</p><label>Technologies <input value={form.technologies.join(', ')} onChange={(event) => setForm({ ...form, technologies: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) })} placeholder="Go, Next.js" /></label><button className="primary-button" type="submit">{project ? 'Save changes' : 'Create project'}</button></form>
